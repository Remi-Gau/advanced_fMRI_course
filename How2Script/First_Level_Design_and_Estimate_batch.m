...
ImagesFolder = fullfile(pwd, TargetDirectory);
...
for j = 1:length(IMAGES_ls)
    matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,1).scans{j,1} = fullfile(ImagesFolder, IMAGES_ls(j).name);
end
...
matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,1).multi{1} = fullfile(pwd, 'all-conditions.mat');
matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,1).multi_reg{1} = fullfile(ImagesFolder, Mov_Parameter_ls.name);
